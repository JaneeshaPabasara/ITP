...
function UserProfile({ onBack, onLogout, goToMyProfile }) {
  ...
  const handleDelete = async () => {
    try {
      const res = await fetch(`http://localhost:5002/api/users/${userId}`, { method: "DELETE" });
  ...
  const handleGeneratePDF = () => {
    ...
    const doc = new jsPDF();
    const currentUsername = localStorage.getItem("firstName") || "Unknown";

    // Header
    doc.setFontSize(22);
    doc.setFont("helvetica", "bold");
    doc.text("HeavySync", doc.internal.pageSize.getWidth() / 2, 20, { align: "center" });
    ...
    entries.forEach((entry, index) => {
      doc.text(`${entry.label}: ${entry.value || "N/A"}`, 20, 40 + index * 10);
    });

    // Footer
    doc.setFontSize(12);
    doc.setFont("helvetica", "italic");
    doc.text(`Generated by: ${currentUsername}`, 20, doc.internal.pageSize.getHeight() - 10);

    doc.save(`${user.username || "user"}_details.pdf`);
  };
            ...
            {isEditing ? (
              ...
              <>
                <button
                  ...
                  className="px-10 py-4 rounded-full bg-gradient-to-r from-indigo-700 to-indigo-900 text-white font-extrabold text-lg shadow-lg hover:scale-105 hover:shadow-indigo-700/80 transition transform duration-300 active:scale-95"
                >
                  Generate PDF
                </button>
                <button
                  onClick={handleDelete}
                  className="px-10 py-4 rounded-full bg-gradient-to-r from-red-700 to-red-900 text-white font-extrabold text-lg shadow-lg hover:scale-105 hover:shadow-red-700/80 transition transform duration-300 active:scale-95"
                >
                  Delete
                </button>
                <button
